...
(1)获取到标志位，计算”存放地址值的内存块“的首地址,Get_Start_Addr
(2)获取”存放地址值的内存块“中的值,Get_Task_Addr
(3)执行具体的任务,DoWork
(4)释放”存放地址值的内存块“指向的内存区域,Free_Task_Addr
*/
int DoWork_Appliction( void * pPoint )
{
	...
	pPoint_Start_Addr = Get_Start_Addr( pPoint );
	if ( NULL == pPoint_Start_Addr )
	{
		return -1;
	}

	Get_Task_Addr( pPoint_Start_Addr, &pPoint_Task_Addr );
