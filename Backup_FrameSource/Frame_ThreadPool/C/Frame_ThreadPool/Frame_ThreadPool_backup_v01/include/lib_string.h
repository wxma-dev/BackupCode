...
char * Th_Str_To_Str( char * sSrc_string, char * sDst_string, char * sTh_Src_String, char * sTh_Dst_String )
{
	...
	/* 字符串的中间长度 */
	iLengthString_Zj = strlen( sTh_Dst_String ) - strlen( sTh_Src_String );
	...
	/* 拷贝至目标字符串 */
	memcpy( sDst_string, pPoint_Mem, iLengthString_Tb + iLengthString_Zj + iLengthString_Wb );
...
void Trim_Before( char * sString ) 
{
	...
	while( 1 )
	{
		...
		else
		{
			...
			/* 源字符串前面有空，则删除空格 */
			strcpy( pPoint_Mem, sString + iCount );
		}
